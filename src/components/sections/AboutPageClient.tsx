...
const LEADERSHIP = [
  {
    name:    'Mrs. Rojina Shrestha',
    title:   'Founder & Chief Executive Officer',
    image:   'https://res.cloudinary.com/dosxengut/image/upload/v1785015615/ceo-Refinedbeauty_t2ke06.png',
    message: 'When I opened the doors to Refined Beauty Hub, I wanted more than a salon — I wanted a place where every person who walks in leaves feeling more like themselves. Beauty has never been about becoming someone new; it is about revealing what was always there. That belief shapes every service we offer and every artist we train.',
  },
  {
    name:    'Mr. Ajay Chemjong',
    title:   'Co-Founder & Chief Operating Officer',
    image:   'https://res.cloudinary.com/dosxengut/image/upload/v1785014876/CooRefinedBeauty_v7lxfd.png',
    message: 'Behind every effortless salon visit is a team that has prepared for it a hundred times over. My work lives in the details you should never have to think about — the hygiene, the timing, the training, the consistency between our branches. Excellence is not a promise we make once; it is a standard we keep every single day.',
  },
]
...
export function AboutPageClient({ staff }: { staff: Staff[] }) {
  return (
    <div style={{ background: '#F9F5F0' }}>
      ...
      {/* ── Leadership — a word from the founders ────────────── */}
      <section className="py-14 lg:py-20" style={{ background: '#F9F5F0' }}>
        <div className="luxury-container">
          <motion.div {...fade()} className="text-center mb-10 lg:mb-12">
            <p className="text-[10px] font-semibold tracking-[0.35em] uppercase mb-3" style={{ color: '#b8976b' }}>
              Leadership
            </p>
            <h2 className="text-4xl lg:text-5xl font-light"
              style={{ fontFamily: 'var(--font-cormorant)', color: '#1a1410' }}>
              A Word From Our <em style={{ color: '#b8976b' }}>Founders</em>
            </h2>
          </motion.div>
            ...
            {LEADERSHIP.map((leader, i) => (
              <motion.div
                ...
                className={`grid gap-6 lg:gap-12 items-center ${
                  i % 2 === 1
                    // Mirror the columns as well as the order, so the portrait
                    // keeps its fixed column whichever side it sits on.
                    ? 'lg:grid-cols-[1fr_300px] lg:[&>*:first-child]:order-2'
                    : 'lg:grid-cols-[300px_1fr]'
                }`}
              >
                {/* Portrait with offset gold frame */}
                <div className="relative mx-auto w-full max-w-[240px] lg:max-w-none">
                  <div className="absolute -inset-3 border rounded-sm hidden lg:block"
                    style={{ borderColor: '#b8976b40' }} />
                  <div className="relative aspect-[4/5] rounded-sm overflow-hidden"
                    style={{ background: '#ede5db' }}>
                    <Image
                      src={leader.image}
                      alt={`${leader.name}, ${leader.title} of Refined Beauty Hub`}
                      fill
                ...
                {/* Message — capped so lines stay a comfortable reading width */}
                <div className="text-center lg:text-left max-w-2xl mx-auto lg:mx-0">
                  <span className="block text-4xl leading-none mb-1 select-none"
                    style={{ fontFamily: 'var(--font-cormorant)', color: '#b8976b', opacity: 0.35 }}
                    aria-hidden="true">
                    &ldquo;
                  </span>

                  <p className="text-sm leading-[1.8] mb-5" style={{ color: '#5c4d43' }}>
                    {leader.message}
                  </p>

                  <div className="w-10 h-px mx-auto lg:mx-0 mb-3.5" style={{ background: '#b8976b' }} />
      ...
      {/* ── Meet the Team — same carousel as the homepage ────── */}
      {staff.length > 0 && (
        <section className="py-20 lg:py-24" style={{ background: '#F9F5F0' }}>
          <div className="luxury-container">
