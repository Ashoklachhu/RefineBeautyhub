...
import {
  X, Search, User, DollarSign, Loader2, ChevronDown,
  Globe, Footprints, Phone, ShieldCheck,
} from 'lucide-react'
import {
  adminSearchUsers,
...
import type { AcademyCourse, Enrollment, BookingSource, Profile } from '@/types/database'

// ── Types ─────────────────────────────────────────────────────

type EnrollmentRow = Enrollment & {
  course?: {
    title: string; category: string; price: number; level: string;
    format: string; duration_text: string; image_url: string | null
  } | null
...
interface EnrollmentFormPanelProps {
  ...
  enrollment?: EnrollmentRow | null   // if set → edit mode
  onClose:    () => void
}
...
function UserSearch({ onSelect }: { onSelect: (u: Profile | null) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      <div className="relative">
        ...
        />
        {loading && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 dark:text-neutral-500 animate-spin" />}
        {selected && !loading && (
          <button onClick={clear} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-neutral-500 hover:text-gray-900 dark:hover:text-white transition-colors">
      ...
      {open && results.length === 0 && !loading && query.length >= 2 && (
        <div className="absolute z-50 top-full mt-1 w-full bg-white dark:bg-neutral-800 border border-gray-200 dark:border-white/10 rounded-lg px-3 py-3 text-sm text-gray-500 dark:text-neutral-400">
          No users found. Enrollment will be created as guest.
        </div>
      )}
    </div>
  )
...
export function EnrollmentFormPanel({ courses, enrollment, onClose }: EnrollmentFormPanelProps) {
  const router    = useRouter()
  const [, start] = useTransition()
  const isEdit    = !!enrollment

  const [selectedUser, setSelectedUser] = useState<Profile | null>(null)
  const [guestName,    setGuestName]    = useState(enrollment?.guest_name  ?? '')
  const [guestEmail,   setGuestEmail]   = useState(enrollment?.guest_email ?? '')
  const [guestPhone,   setGuestPhone]   = useState(enrollment?.guest_phone ?? '')
  const [useGuest,     setUseGuest]     = useState(!enrollment?.user_id && !!(enrollment?.guest_name))
  const [courseId,     setCourseId]     = useState(enrollment?.course_id ?? '')
  const [amount,       setAmount]       = useState(enrollment ? String(enrollment.amount_paid) : '')
  const [source,       setSource]       = useState<BookingSource>(
    (enrollment as (EnrollmentRow & { source?: BookingSource }) | undefined)?.source ?? 'admin'
  )
  ...
  function handleCourseChange(id: string) {
    ...
    if (course && !amount) setAmount(String(course.price))
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    start(async () => {
      if (isEdit && enrollment) {
        const { error } = await adminUpdateEnrollment(enrollment.id, {
          course_id:   courseId,
        ...
        const { error } = await adminCreateEnrollment({
          user_id:     userId,
          guest_name:  gName,
          guest_email: gEmail,
          guest_phone: gPhone,
      ...
      <div className="fixed top-0 right-0 bottom-0 z-50 w-full max-w-lg bg-slate-50 dark:bg-neutral-950 border-l border-gray-200 dark:border-white/10 shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-200 dark:border-white/10 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-neutral-400 mt-0.5">
              {isEdit ? `Editing ${enrollment?.reference}` : 'Manually create an enrollment'}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="px-6 py-5 space-y-5">
            ...
            {/* ── Source ── */}
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-neutral-400 mb-2">Enrollment Source</label>
            ...
            {!isEdit && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs font-medium text-gray-500 dark:text-neutral-400">Student</label>
                  <button
                    type="button"
            ...
            {/* ── Course ── */}
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-neutral-400 mb-2">Course *</label>
              <div className="relative">
                <select
                  value={courseId}
                ...
                >
                  ...
                  {courses.map(c => (
                    <option key={c.id} value={c.id}>
                      {c.title} · NPR {c.price.toLocaleString()} · {c.duration_text}
                    </option>
                  ))}
                </select>
              ...
              {selectedCourse && (
                <div className="mt-2 px-3 py-2.5 rounded-lg bg-gray-50 dark:bg-neutral-800/60 border border-gray-200 dark:border-white/5 text-xs text-gray-500 dark:text-neutral-400 space-y-0.5">
                  <p className="text-gray-900 dark:text-white font-medium">{selectedCourse.title}</p>
                  <p>{selectedCourse.category} · {selectedCourse.level} · {selectedCourse.format.replace('_', '-')} · {selectedCourse.duration_text}</p>
                </div>
              )}
            </div>
            ...
            {/* ── Amount ── */}
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-neutral-400 mb-2">Amount Paid (NPR) *</label>
              <div className="relative">
            ...
            {/* ── Notes ── */}
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-neutral-400 mb-2">Notes</label>
              <textarea
                value={notes}
